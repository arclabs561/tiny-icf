...
def test_model_forward_return_features():
    """Test that return_features=True returns a stable, self-consistent payload."""
    model = UniversalICF()

    batch_size = 4
    max_length = 20
    byte_tensors = torch.randint(0, 256, (batch_size, max_length), dtype=torch.long)

    output, features = model(byte_tensors, return_features=True)

    assert output.shape == (batch_size, 1)
    assert isinstance(features, dict)

    assert "icf_score" in features
    assert "raw_output" in features
    assert "confidence" in features
    assert "feature_activations" in features

    assert features["icf_score"].shape == (batch_size, 1)
    assert features["raw_output"].shape == (batch_size, 1)
    assert features["confidence"].shape == (batch_size, 1)
    assert features["feature_activations"].shape[0] == batch_size

    # Output must match the returned score and be in [0, 1].
    assert torch.allclose(output, features["icf_score"])
    assert torch.all(output >= 0.0) and torch.all(output <= 1.0)

    # Confidence is a probability-like score.
    assert torch.all(features["confidence"] >= 0.0) and torch.all(features["confidence"] <= 1.0)
