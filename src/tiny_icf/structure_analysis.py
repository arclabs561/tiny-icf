...
def compute_ngram_icf_correlation(
    ...
    return {
        ...
        "top_ngrams": sorted(ngram_avg_icf.items(), key=lambda x: x[1], reverse=True)[:20],
    }
...
def estimate_structure_strength(
    ...
    compression_potential = (
        actual_size_bits / theoretical_min_bits if theoretical_min_bits > 0 else 0.0
    )
    ...
    results["interpretation"] = (
        "strong" if structure_strength > 0.7 else "moderate" if structure_strength > 0.4 else "weak"
    )
