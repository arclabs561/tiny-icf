...
def compute_prediction_distances(
    ...
    relative_errors = np.where(
        targets_np != 0,
        abs_errors / (np.abs(targets_np) + 1e-8),
        abs_errors
    )
    ...
    if non_zero_mask.sum() > 0:
        percent_close_10 = np.mean(abs_errors[non_zero_mask] < 0.1 * np.abs(targets_np[non_zero_mask])) * 100
        percent_close_20 = np.mean(abs_errors[non_zero_mask] < 0.2 * np.abs(targets_np[non_zero_mask])) * 100
        percent_close_50 = np.mean(abs_errors[non_zero_mask] < 0.5 * np.abs(targets_np[non_zero_mask])) * 100
    else:
        percent_close_10 = percent_close_20 = percent_close_50 = 0.0
    
    # Additional: Percent within 1%, 5% for high-precision analysis
    if non_zero_mask.sum() > 0:
        percent_close_1 = np.mean(abs_errors[non_zero_mask] < 0.01 * np.abs(targets_np[non_zero_mask])) * 100
        percent_close_5 = np.mean(abs_errors[non_zero_mask] < 0.05 * np.abs(targets_np[non_zero_mask])) * 100
    else:
...
def find_interesting_cases(
    ...
    for idx in close_indices[:50]:  # Limit to 50 examples
        ...
        cases['close_calls'].append((
            word,
            float(predictions_np[idx]),
            float(targets_np[idx]),
            float(abs_errors[idx])
        ))
...
def format_diagnostic_report(
    ...
    if cases['false_positives']:
        lines.append(f"  False Positives (predicted high, actually low, top {min(top_n, len(cases['false_positives']))}):")
        for word, pred, target, error in cases['false_positives'][:top_n]:
            lines.append(f"    {word:20s} pred={pred:.4f} target={target:.4f} error={error:.4f}")
        lines.append("")
    
    if cases['false_negatives']:
        lines.append(f"  False Negatives (predicted low, actually high, top {min(top_n, len(cases['false_negatives']))}):")
    ...
    if cases['ranking_errors']:
        ...
        for word1, pred1, target1, word2, pred2, target2, diff in cases['ranking_errors'][:top_n]:
            lines.append(f"    {word1:15s} (pred={pred1:.3f}, tgt={target1:.3f}) vs {word2:15s} (pred={pred2:.3f}, tgt={target2:.3f}) [diff={diff:.3f}]")
        lines.append("")
    ...
    if 'by_icf_range' in patterns:
        ...
        for range_name, stats in patterns['by_icf_range'].items():
            lines.append(f"  {range_name:15s} (n={stats['count']:4d}): "
    ...
    if 'by_length' in patterns:
        ...
        for length in sorted(patterns['by_length'].keys())[:10]:  # Top 10 lengths
            ...
            lines.append(f"  length={length:2d} (n={stats['count']:4d}): "
                        f"mean_err={stats['mean_error']:.4f}, "
