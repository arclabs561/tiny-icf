...
class LanguageModelTeacher:
    ...
    def get_intermediate_features(self, words: list[str], layer_idx: Optional[int] = None) -> torch.Tensor:
        """
    ...
    def forward(
        ...
        if self.use_feature_distillation and student_features is not None and teacher_features is not None:
            ...
            feature_loss = 1.0 - F.cosine_similarity(
                student_features_norm, teacher_features_norm, dim=1
            ).mean()
        ...
        if words is not None:
            with torch.no_grad():
                if self.teacher_model.use_word_frequency_head:
                    ...
                    teacher_predictions = teacher_predictions / teacher_predictions.max()  # Normalize to [0, 1]
