...
class RealTimeNormalizedLoss(nn.Module):
    ...
    def forward(
        ...
        if self.use_neural_ndcg:
            ...
            self.neural_ndcg_ema = self.ema_decay * self.neural_ndcg_ema + (1 - self.ema_decay) * ndcg_loss.detach()
        ...
        if self.use_listwise_ranking:
            ...
            self.listwise_ema = self.ema_decay * self.listwise_ema + (1 - self.ema_decay) * listwise_loss.detach()
        ...
        if pairs is not None and len(pairs) > 0:
            ...
            self.ranking_ema = self.ema_decay * self.ranking_ema + (1 - self.ema_decay) * rank.detach()
...
class UncertaintyWeightedLoss(nn.Module):
    ...
    def forward(
        ...
        if self.use_listwise_ranking:
            ...
            listwise_weighted = (1.0 / (2.0 * sigma_listwise_sq)) * listwise_loss + self.log_sigma_listwise
