...
try:
    ...
    HAS_RANK_EVAL = False
    warnings.warn("rank-eval not available. Install with: pip install rank-eval or build from ../rank-eval/rank-eval-python")
...
def compute_ranking_metrics(
    ...
    if use_graded:
        ...
        if max_target > min_target:
            relevance_scores = ((targets - min_target) / (max_target - min_target) * 3.0).astype(int)
        else:
            relevance_scores = np.zeros_like(targets, dtype=int)
        
        # Create ranked list: (index, prediction_score)
        indices = np.arange(len(predictions))
        ranked: List[Tuple[str, float]] = [(str(i), float(pred)) for i, pred in zip(indices, predictions)]
