...
def ranking_loss(
    ...
    pred2: torch.Tensor,
    margin: float = 0.1,
    target_diff: Optional[torch.Tensor] = None,
    smooth: bool = True,
...
def neural_ndcg_loss_simple(
    ...
    positions = torch.arange(
        1, len(top_k_relevance) + 1, dtype=torch.float32, device=predictions.device
    )
...
def spearman_loss_torchsort(
    ...
    pred_ranks = torchsort.soft_rank(
        predictions.unsqueeze(0), regularization_strength=regularization_strength
    )
    target_ranks = torchsort.soft_rank(
        targets.unsqueeze(0), regularization_strength=regularization_strength
    )
...
def spearman_loss_diffsort(
    ...
    if next_power > batch_size:
        ...
        target_padded = torch.cat(
            [targets, torch.full((pad_size,), min_target, device=targets.device)]
        )
    else:
...
class SpearmanLoss(nn.Module):
    ...
    def forward(
        ...
        if self.backend == "rank-relax":
            ...
            return rank_relax_spearman_loss(predictions, targets, self.regularization_strength)
        elif self.backend == "torchsort":
            return spearman_loss_torchsort(predictions, targets, self.regularization_strength)
        elif self.backend == "diffsort":
            return spearman_loss_diffsort(predictions, targets, self.steepness)
        else:  # built-in
            if self.use_vectorized:
                return spearman_loss_vectorized(predictions, targets, self.regularization_strength)
            else:
                return spearman_loss_simple(predictions, targets, self.regularization_strength)
        ...
        if use_spearman:
            try:
                ...
                if backend_info["current"] == "rank-relax":
                    print(
                        "✅ Spearman loss using rank-relax (optimized Rust, analytical gradients)"
                    )
                elif backend_info["current"] == "torchsort":
                    print("✅ Spearman loss using torchsort (O(n log n), best performance)")
                elif backend_info["current"] == "diffsort":
                    print("✅ Spearman loss using diffsort (O(n²(log n)²))")
                else:
                    print("✅ Spearman loss using built-in (O(n²), no dependencies)")
    ...
    def forward(
        ...
        if self.use_listwise_ranking:
            if self.listwise_method == "lambdarank":
                ...
                listwise_loss = approx_ndcg_loss(
                    pred_1d, target_1d, temperature=self.listwise_temperature
                )
            else:
    ...
    def get_component_stats(self) -> Dict[str, float]:
        ...
        total_mean = sum(
            [
                ...
                stats.get("neural_ndcg_mean", 0)
                * (self.neural_ndcg_weight if self.use_neural_ndcg else 0),
                stats.get("listwise_mean", 0)
                * (self.listwise_weight if self.use_listwise_ranking else 0),
        ...
        if total_mean > 0:
            ...
            if self.use_neural_ndcg:
                stats["neural_ndcg_ratio"] = (
                    stats.get("neural_ndcg_mean", 0) * self.neural_ndcg_weight
                ) / total_mean
            if self.use_spearman:
            ...
            if self.use_listwise_ranking:
                stats["listwise_ratio"] = (
                    stats.get("listwise_mean", 0) * self.listwise_weight
                ) / total_mean
