...
def main():
    ...
    parser.add_argument(
        "--early-stopping", type=int, default=10, help="Patience for early stopping"
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        val_loss, val_metrics = validate(
            model, val_loader, criterion, device, use_amp, compute_metrics=True
        )
        ...
        if val_metrics:
            ...
            if val_metrics.get("spearman_corr", 0) != 0:
                print(
                    f"  Spearman: {val_metrics['spearman_corr']:.4f} (p={val_metrics['spearman_p']:.4f})"
                )
            print(
                f"  Pred range: [{val_metrics['pred_min']:.4f}, {val_metrics['pred_max']:.4f}], std: {val_metrics['pred_std']:.4f}"
            )
