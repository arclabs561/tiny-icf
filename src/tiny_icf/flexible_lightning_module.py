...
class FlexibleIDFLightningModule(LightningModule):
    ...
    def __init__(
        ...
        if config.get('use_multi_task_model', False):
            ...
            if config.get('model_type') == 'transformer':
                ...
                try:
                    ...
                    print(f"⚠️  CharacterLevelTransformerICF not available: {e}, falling back to UniversalICF")
                    self.model = UniversalICF()
        ...
        use_research_aligned = config.get('use_research_aligned_loss', False) and HAS_RESEARCH_ALIGNED_LOSS
        use_unified_loss = config.get('use_unified_loss', False) and HAS_UNIFIED_LOSS and not use_research_aligned
        ...
        if use_research_aligned:
            ...
            self.criterion = ResearchAlignedICFLoss(
                ...
                ranking_method=config.get('ranking_method', 'sigmoid'),  # "sigmoid", "neural_sort", "probabilistic", "smooth_i"
            ...
            self.criterion = CombinedLoss(
                ...
                rank_weight=config.get('rank_weight', 0.01),  # Use config value (default 0.01 for magnitude normalization)
        ...
        if self.use_distillation:
            try:
                ...
                self.distillation_loss_fn = DistillationLoss(
                    ...
                    use_dynamic_temperature=config.get('use_dynamic_temperature', True),  # Enable by default
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_distillation and teacher_predictions is not None:
            ...
            self.log("train_loss_supervised", loss_components['supervised_loss'], on_step=True, on_epoch=True)
            self.log("train_loss_distillation", loss_components['distillation_loss'], on_step=True, on_epoch=True)
            if loss_components['feature_loss'].item() > 0:
                self.log("train_loss_feature", loss_components['feature_loss'], on_step=True, on_epoch=True)
            # Log temperature if dynamic
            if 'temperature' in loss_components:
                self.log("train_temperature", loss_components['temperature'], on_step=True, on_epoch=True)
            ...
            if pairs is not None and len(pairs) > 0:
                ...
                pair_target_diffs = pair_target_diffs.to(predictions.device) if pair_target_diffs is not None else None
            ...
            loss_result = self.criterion(
                ...
                language_logits=model_outputs.get('language') if self.use_multi_task_model and isinstance(model_outputs, dict) else (batch.get('language_logits') if isinstance(batch, dict) else None),
                language_targets=batch.get('language_targets') if isinstance(batch, dict) else None,
                era_logits=model_outputs.get('era') if self.use_multi_task_model and isinstance(model_outputs, dict) else (batch.get('era_logits') if isinstance(batch, dict) else None),
                era_targets=batch.get('era_targets') if isinstance(batch, dict) else None,
                current_predictions=predictions,
                current_targets=icf_targets,
                historical_predictions=batch.get('historical_predictions') if isinstance(batch, dict) else None,
                historical_targets=batch.get('historical_targets') if isinstance(batch, dict) else None,
                word_icf_scores=batch.get('word_icf_scores') if isinstance(batch, dict) else None,
                original_embedding=batch.get('original_embedding') if isinstance(batch, dict) else None,
                reduced_embedding=batch.get('reduced_embedding') if isinstance(batch, dict) else None,
    ...
    def on_before_optimizer_step(self, optimizer):
        ...
        if self.clip_grad_norm is not None and self.clip_grad_norm > 0:
            grad_norm = torch.nn.utils.clip_grad_norm_(self.parameters(), max_norm=self.clip_grad_norm)
    ...
    def on_validation_epoch_end(self):
            ...
            try:
                ...
                if isinstance(self.criterion, ResearchAlignedICFLoss):
                    ...
                    _, loss_components = self.criterion(
                        ...
                        sample_targets.unsqueeze(1) if sample_targets.dim() == 1 else sample_targets,
                    ...
                    spearman_ratio = metrics['spearman_corr'] / theoretical_bound if theoretical_bound > 0 else 0.0
                    ...
                    component_bounds = {
                        ...
                        'language_classification': {'good': 0.7, 'poor': 1.0, 'best': 0.5},  # Cross-entropy loss
                        'language_ranking': {'good': 0.1, 'poor': 0.2, 'best': 0.0},
                        'era_classification': {'good': 1.0, 'poor': 1.5, 'best': 0.5},  # Cross-entropy loss
                        ...
                        'text_reduction_regret': {'good': 0.30, 'poor': 0.50, 'best': 0.15},  # Cosine distance
                        'text_reduction_path_regret': {'good': 0.40, 'poor': 0.60, 'best': 0.20},  # Cumulative path regret
                    ...
                    for component_name, component_value in loss_components.items():
                        ...
                        if component_name in component_bounds:
                            ...
                            self.log(f"val_loss_{component_name}_status", status_code, prog_bar=False)
                            
                            # Log ratio to good threshold
                            if bounds['good'] > 0:
                                ratio = component_value / bounds['good']
                                self.log(f"val_loss_{component_name}_vs_good", ratio, prog_bar=False)
                            
                            # Log ratio to best threshold (if available)
                            if 'best' in bounds and bounds['best'] > 0:
                                ratio_best = component_value / bounds['best']
                                self.log(f"val_loss_{component_name}_vs_best", ratio_best, prog_bar=False)
        ...
        try:
            ...
            current_epoch = self.current_epoch if hasattr(self, 'current_epoch') else getattr(self.trainer, 'current_epoch', 0)
            if current_epoch % 5 == 0 or current_epoch == 0:
                ...
                if hasattr(self.trainer, 'log_dir') and self.trainer.log_dir:
                    ...
                    report_path = os.path.join(
                        ...
                        f"diagnostic_report_epoch_{current_epoch}.txt"
                    )
                    ...
                    json_path = os.path.join(
                        self.trainer.log_dir,
                        f"diagnostic_data_epoch_{current_epoch}.json"
                    )
                    ...
                    diagnostic_json = {
                        ...
                        'interesting_cases': {
                            ...
                            'ranking_errors': [
                                {
                                    'word1': w1, 'pred1': float(p1), 'target1': float(t1),
                                    'word2': w2, 'pred2': float(p2), 'target2': float(t2),
                                    'diff': float(d)
                                }
                        ...
                        'error_patterns': {
                            'by_icf_range': {
                                ...
                                for k, v in diagnostics['error_patterns'].get('by_icf_range', {}).items()
                            },
                            'by_length': {
                                str(k): {kk: float(vv) if isinstance(vv, (np.floating, np.integer)) else vv
                                         for kk, vv in v.items()}
                                for k, v in diagnostics['error_patterns'].get('by_length', {}).items()
                            },
                        },
                    }
                    ...
                    if self._aim_logger is not None:
                        try:
                            ...
                            if run is not None:
                                ...
                                run.track(aim.Text(report), name='diagnostic_report', step=current_epoch, context={'subset': 'validation', 'epoch': current_epoch})
                                
                                # Log diagnostic summary metrics individually (Aim prefers individual metrics)
                                run.track(len(cases['worst_offenders']), name='diagnostic_worst_offenders_count', step=current_epoch, context={'subset': 'validation'})
                                run.track(len(cases['close_calls']), name='diagnostic_close_calls_count', step=current_epoch, context={'subset': 'validation'})
                                run.track(len(cases['false_positives']), name='diagnostic_false_positives_count', step=current_epoch, context={'subset': 'validation'})
                                run.track(len(cases['false_negatives']), name='diagnostic_false_negatives_count', step=current_epoch, context={'subset': 'validation'})
                                run.track(len(cases['ranking_errors']), name='diagnostic_ranking_errors_count', step=current_epoch, context={'subset': 'validation'})
                        except Exception as e:
    ...
    def configure_optimizers(self):
        ...
        scheduler_type = self.config.get('scheduler_type', 'cosine')  # 'cosine', 'plateau', or 'cosine_warmup'
