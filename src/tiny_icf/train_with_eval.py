...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("models/model_with_eval.pt"), help="Output model path")
    ...
    for epoch in range(args.epochs):
        ...
        eval_result = evaluate_mid_training(model, val_loader, device, epoch + 1, args.eval_interval)
        if eval_result:
            ...
            print(f"  Predictions: mean={pred_stats['mean']:.4f}, std={pred_stats['std']:.4f}, range=[{pred_stats['min']:.4f}, {pred_stats['max']:.4f}]")
