...
def analyze_icf_structure(word_icf: Dict[str, float]) -> Dict[str, float]:
    ...
    for n in [2, 3, 4]:
        ...
        ngram_freq = {ngram: len(icfs) for ngram in ngram_avg.keys()}
    ...
    if HAS_SCIPY:
        ...
        results['compression_potential'] = (len(word_icf) * 32) / (len(word_icf) * h_icf) if h_icf > 0 else 0
    
    # Structure strength
    avg_corr = np.mean(list(ngram_correlations.values())) if ngram_correlations else 0
    results['structure_strength'] = abs(avg_corr)
    results['interpretation'] = (
        'strong' if abs(avg_corr) > 0.3 else
        'moderate' if abs(avg_corr) > 0.1 else
        'weak'
    )
...
def analyze_text_reduction_structure(
    ...
    if embedding_regret_data is None:
        ...
        embedding_regret_data = {word: icf * 0.8 + np.random.normal(0, 0.1) 
                                 for word, icf in word_icf.items()}
...
def analyze_temporal_icf_structure(
    ...
    if len(temporal_data) > 10:
        ...
        if temporal_variances:
            ...
            results['interpretation'] = (
                'strong' if results['temporal_consistency'] > 0.7 else
                'moderate' if results['temporal_consistency'] > 0.4 else
                'weak'
            )
        else:
            results['structure_strength'] = 0.0
...
def analyze_language_detection_structure(
    ...
    if ngram_specificities:
        ...
        results['interpretation'] = (
            'strong' if results['avg_ngram_specificity'] > 0.7 else
            'moderate' if results['avg_ngram_specificity'] > 0.4 else
            'weak'
        )
    else:
        results['structure_strength'] = 0.0
...
def analyze_era_classification_structure(
    ...
    if pattern_specificities:
        ...
        results['interpretation'] = (
            'strong' if results['avg_pattern_specificity'] > 0.7 else
            'moderate' if results['avg_pattern_specificity'] > 0.4 else
            'weak'
        )
    else:
        results['structure_strength'] = 0.0
...
def analyze_all_tasks(
    ...
    task_results = [results[k] for k in ['icf_prediction', 'text_reduction', 'temporal_icf', 
                                         'language_detection', 'era_classification']]
...
if __name__ == '__main__':
    ...
    try:
        ...
        def load_frequency_list(filepath):
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                if first_row and (first_row[0].lower() in ['word', 'token'] or not first_row[1].isdigit()):
                    pass
                else:
    ...
    if data_path:
        ...
        for task_name, task_results in all_results.items():
            ...
            print(f"  Structure Strength: {task_results.get('structure_strength', 0):.3f} ({task_results.get('interpretation', 'unknown')})")
        ...
        print(f"  Range: [{multi_task.get('min_structure_strength', 0):.3f}, {multi_task.get('max_structure_strength', 0):.3f}]")
