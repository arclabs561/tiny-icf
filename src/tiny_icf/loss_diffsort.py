...
def spearman_loss_diffsort(
    ...
    if batch_size <= 16:
        ...
        pred_ranks = torch.argsort(
            torch.argsort(predictions, descending=False), descending=False
        ).float()
        target_ranks = torch.argsort(
            torch.argsort(targets, descending=False), descending=False
        ).float()
...
def spearman_loss_fast_soft_sort(
    ...
    if soft_rank is None:
        raise ImportError(
            "fast-soft-sort not installed. Install from: https://github.com/google-research/fast-soft-sort"
        )
    ...
    # Get soft ranks
    pred_ranks = soft_rank(pred_reshaped, regularization_strength=regularization_strength)[
        0
    ]  # [Batch]
    target_ranks = soft_rank(target_reshaped, regularization_strength=regularization_strength)[
        0
    ]  # [Batch]
...
class DifferentiableSortingLoss(nn.Module):
    ...
    def __init__(
        ...
        if method == "diffsort":
            ...
            if _try_import_fast_soft_sort()[0] is None:
                raise ImportError(
                    "fast-soft-sort not installed. See: https://github.com/google-research/fast-soft-sort"
                )
        else:
    ...
    def forward(
        ...
        huber = F.smooth_l1_loss(
            predictions_flat, targets_flat, reduction="mean", beta=self.huber_delta
        )

        # Spearman loss using differentiable sorting
        if self.method == "diffsort":
            spearman_loss = spearman_loss_diffsort(predictions, targets, steepness=self.steepness)
        else:  # fast_soft_sort
            spearman_loss = spearman_loss_fast_soft_sort(
                predictions, targets, regularization_strength=self.regularization_strength
            )
