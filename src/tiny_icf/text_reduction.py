...
def reduce_text_with_icf(
    ...
    # Reduce
    if method == "greedy":
        reduced_words, regret = greedy_icf_reduction(words, icf_scores, target_length, embedding_fn)
    elif method == "dynamic":
    ...
    stats = {
        ...
        "avg_icf_dropped": (
            np.mean([icf for i, icf in enumerate(icf_scores) if words[i] not in reduced_words])
            if len(reduced_words) < n_words
            else 0.0
        ),
        "avg_icf_kept": np.mean(
            [icf for i, icf in enumerate(icf_scores) if words[i] in reduced_words]
        ),
        "regret": regret,
    }
