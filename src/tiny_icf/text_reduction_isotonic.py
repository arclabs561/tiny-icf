...
class IsotonicTextReducer:
    ...
    def reduce_isotonic(
        ...
        current_embedding = original_embedding.clone()
        previous_regret = 0.0
        ...
        while len(current_words) > target_length:
            ...
            current_embedding = self.compute_embedding(' '.join(current_words))
        ...
        is_isotonic = all(
            regret_curve[i] >= regret_curve[i-1] 
            for i in range(1, len(regret_curve))
        )
        ...
        stats = {
            ...
            'avg_icf_removed': np.mean([p['icf_removed'] for p in progression]) if progression else 0.0,
            'regret': final_regret,
            'steps': step,
