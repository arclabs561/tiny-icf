...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("model_curriculum.pt"), help="Output model path"
    )
    parser.add_argument(
        "--curriculum-stages", type=int, default=5, help="Number of curriculum stages"
    )
    parser.add_argument(
        "--warmup-epochs", type=int, default=5, help="Warmup epochs (easy words only)"
    )
    parser.add_argument(
        "--typo-corpus", type=Path, help="Path to real typo corpus CSV (typo,correction)"
    )
    parser.add_argument("--emoji-freq", type=Path, help="Path to emoji frequency CSV")
    parser.add_argument(
        "--multilingual", action="store_true", help="Use multilingual ICF computation"
    )
    parser.add_argument(
        "--include-symbols", action="store_true", default=True, help="Include symbol augmentation"
    )
    parser.add_argument(
        "--include-emojis", action="store_true", default=True, help="Include emoji augmentation"
    )
    ...
    print(
        f"Curriculum learning: {args.curriculum_stages} stages, {args.warmup_epochs} warmup epochs"
    )
