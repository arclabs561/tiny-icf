...
class ResidualICF(nn.Module):
    ...
    def __init__(
        ...
        self.head_residual_proj = (
            nn.Linear(conv_channels * 9, hidden_dim)
            if conv_channels * 9 != hidden_dim
            else nn.Identity()
        )
        ...
        for module in [self.head_linear1, self.head_residual_proj]:
            if isinstance(module, nn.Linear):
                nn.init.kaiming_uniform_(
                    module.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu"
                )
        ...
        for conv_block in [self.conv3_block, self.conv5_block, self.conv7_block]:
            for layer in conv_block:
                if isinstance(layer, nn.Conv1d):
                    nn.init.kaiming_uniform_(
                        layer.weight, a=math.sqrt(5), mode="fan_in", nonlinearity="relu"
                    )
