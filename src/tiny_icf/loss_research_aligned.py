...
def soft_rank_with_method_adaptive(
    ...
    if values.dim() == 1:
        ...
        ranks_list = rank_relax.soft_rank_with_method(values_list, reg_strength, method=method)
        ...
        for i in range(batch_size):
            ...
            ranks_i = rank_relax.soft_rank_with_method(values_i, reg_strength, method=method)
            ranks_list.append(ranks_i)

        ranks = torch.stack(
            [torch.tensor(r, device=device, dtype=dtype, requires_grad=True) for r in ranks_list]
        )
...
def monotonicity_loss(
    ...
    for feature_name, direction in constraints.items():
        ...
        denominator = (
            torch.sqrt((feature_centered**2).sum()) * torch.sqrt((pred_centered**2).sum()) + 1e-8
        )
...
class ResearchAlignedICFLoss(nn.Module):
    ...
    def forward(
        ...
        if pairs is not None and len(pairs) > 0:
            ...
            magnitude_weight = torch.softmax(
                target_diff_abs / (target_diff_abs.mean() + 1e-8), dim=0
            )
            ...
            if self.use_focal:
                ...
                focal_weight = (
                    1.0 + error_magnitude * (1.0 - pred_order_correct.float())
                ) ** self.focal_gamma
        ...
        if self.use_monotonicity and features is not None:
            mono_loss = monotonicity_loss(predictions, features, self.monotonicity_constraints)
