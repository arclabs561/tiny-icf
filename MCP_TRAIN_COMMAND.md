...
## One Command: Train → Monitor → Results

Use this single MCP command to handle everything:
...
- Checks if training is already running
- Skips redundant steps

✅ **Single Command**: One MCP command does everything
...
Just run the MCP command above. It will:
1. Check if model exists → show results if complete
2. Check if training running → monitor if in progress
3. Otherwise → install, train, monitor, show results

## Alternative: Script Version

If you prefer a script file, use:
```
Execute on RunPod pod w4857gh85qldts: bash /workspace/tiny-icf/scripts/train_or_monitor.sh
```

(After uploading the script file first)
