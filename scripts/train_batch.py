...
# ///
"""
Non-interactive batch training script for RunPod.
PEP 723 script - run with: uv run scripts/train_batch.py
"""
...
try:
    ...
    HAS_LIGHTNING = False
    print("⚠️  PyTorch Lightning not available, falling back to optimized vanilla training")
    print("   Install with: uv pip install lightning")
...
def main():
    parser = argparse.ArgumentParser(description="Batch training entrypoint (PEP 723)")
    parser.add_argument("--data", type=Path, default=Path("data/multilingual/multilingual_combined.csv"))
    parser.add_argument("--output-dir", type=Path, default=Path("/workspace/models"))
    parser.add_argument("--typo-corpus", type=Path, default=Path("data/typos/github_typos.csv"))
    ...
    parser.add_argument("--no-lightning", action="store_true", help="Use vanilla training instead")
    
    args = parser.parse_args()
    
    # Fallback to vanilla if Lightning not available
    if not HAS_LIGHTNING or args.no_lightning:
        print("Using optimized vanilla training (train_optimized.py)")
        from tiny_icf.train_optimized import main as vanilla_main
        
        # Build args list for vanilla training
        vanilla_args = [
            "--data", str(args.data),
            "--output", str(args.output_dir / "model_optimized.pt"),
            "--typo-corpus", str(args.typo_corpus) if args.typo_corpus.exists() else "",
            "--epochs", str(args.epochs),
            "--batch-size", str(args.batch_size),
            "--lr", str(args.lr),
            "--num-workers", str(args.num_workers),
            "--curriculum-stages", str(args.curriculum_stages),
            "--early-stopping", str(args.early_stopping_patience),
        ]
        if args.multilingual:
            vanilla_args.append("--multilingual")
        if args.include_symbols:
            vanilla_args.append("--include-symbols")
        if args.include_emojis:
            vanilla_args.append("--include-emojis")
        
        # Remove empty strings
        vanilla_args = [a for a in vanilla_args if a]
        
        # Save original argv and restore after
        original_argv = sys.argv[:]
        try:
            sys.argv = ["train_optimized.py"] + vanilla_args
            return vanilla_main()
        finally:
    ...
    datamodule = IDFDataModule(
        data_path=args.data,
        typo_corpus_path=args.typo_corpus if args.typo_corpus.exists() else None,
        emoji_freq_path=None,
    ...
    print("🚀 Starting Lightning batch training...")
    ...
    print(f"\n{'='*60}")
    print("✅ Batch training job completed")
